...
**The decision above stands unchanged. The question it deferred — which the modules put in their own
words, "how they compose on screen is a layout decision owned by the Stage 2 caller"
(`station.ts:16`, `pathBuilders.ts:14`) — now has an answer**, recorded here so a reader who stops at
this file learns it rather than re-deriving it.

The caller is `src/components/educraft/line/frames.ts` (Stage 2 Task 2), and **the reconciliation is
viewBox selection, not arithmetic** — which is what makes the note in `pathBuilders.ts` ("there is no
per-point arithmetic that maps one onto the other", `pathBuilders.ts:13`) a non-obstacle rather than
a dead end. A vertical act renders `ACT_VIEW_BOX` (`0 0 1 1`) over a box sized to its own band, so
the act-local anchors *are* viewBox coordinates verbatim and `anchors.ts` needs no transform. The
walk renders `0 0 N 1` over an element `N × 100vw` wide and `100vh` tall, so one unit is one viewport
and `stationPositions`' `x = i` (track widths) becomes the centre of slot `i` with a single
**half-slot offset** — the whole of the transform, and it lives in the caller, as R8 requires.
`station.ts` is unmodified.

**Nothing renders this yet, so no symptom has been "measured" in paint.** `frames.ts` has no caller
of its own until Task 11, and the page and the bundle are unchanged until then. What has been
measured is arithmetic over the strings the module returns: the walk's rail now tiles `0 … N` of its
own `0 0 N 1` frame — 100% of it — where feeding `stationPositions` to `LineStage`'s default
`1200×800` box gave a rail 4 units wide, 0.33% of the box. (The Context's `0.26px × 1.13px` measured
Stage 1's geometry, `origin.exit` at `y = 1`; Task 1 moved that anchor to `y = 0.85`, so at this
commit the same arc measures `0.26px × 0.96px`.)
...
**Task 11** (the page), which is the call site Stage 1 left open.

Each of the consequences below the decision now has an outcome — one of them planned rather than
executed:
...
- **"The walk's draw disagrees with the spec's tween by (N−1)/N"** — **planned, not executed:** the
  fix is to change the *tween*, which is the side this file said needed revisiting, so the walk
  tweens N viewports rather than `N − 1` (Tasks 6–7, neither of which has run yet). `walkFrame`'s
  half is in place and is what forces it: N one-unit segments with station `i` at the centre of slot
  `i`, i.e. at `(i + 0.5) / N` of the frame — *not* `i/N`, which is where its slice starts, as the
  paragraph above already says. `frames.test.ts` asserts the midpoint directly, and the tween this
  resolves to is the one Stage 1's ruling had already offered.
- `stages/README.md`'s **⚠ join warning has been replaced by the outcome**: the join now lives in
  `frames.ts`, and the note that replaced the warning records that nothing draws a strand until the
  page's call site. The mitigation sentence above therefore now points at a note rather than a
