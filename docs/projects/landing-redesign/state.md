...
**A repair round followed Task 11**, at the owner's instruction, after three dispatched reviews found
the stage broken in ways the suite could not see: the strand's box was not the act's for three of the
five acts (gaps of 400–800px at the seams and a 116px sideways jog), Act 1's stations computed to
**500vw** wide so stations 1–4 never rendered, its rail sat below the fold for the whole pinned walk
and its mobile buttons were dead, two GSAP tweens fought over the ribbon's paths and neither drew the
convergence, the fork never drew outside desktop, and no test in the repo could see the page at all.
Nine commits, `a6e71ad`..`8f9c440`; plan `.claude/plans/landing-redesign-stage-2-repair.md`; report
`.stage/sdd/landing-redesign-stage-2/reports/stage-2-repair.md`; rulings R22–R24 in §4 below. Every
one of those defects passed `tsc && lint && test && build`, which is the lesson worth keeping.
...
- **Act 4's brief asks for two things that cannot both hold (R20).** The path runs to `doors.exit`, which the seam contract fixes at the act's **mid-band** (`y = 0.5`), and the same paragraph wants the line to "visibly end in the button". It cannot: `FinalCTA` renders eyebrow → `h2` → lede → the CTA row, so the button is below the band's top by that whole stack. The act ships the contract, with the node at the button's edge; **the alignment is the owner's QA — the plan's own item 10**, so the intent is not in doubt. Options if it is ruled wrong: move `doors.exit` (a Task 1 anchor change), size the strand's box to end at the button, or accept the gap.
...
- **The strand's box is the act's band, and that is a seam fact, not styling.** `inset-0` over the `<section>` makes act-local `y = 0` the act's top edge, where the previous act's exit lands. Way's box *was* the content div, so its strand began below its header — a seam gap at Act 2's top edge, observed at Task 10 and **fixed by the repair** (R22).
...
- **The act's `<section>` is the strand's box — structurally now, not by recollection (R22).** `ActSection` owns a `line` slot rendered as the section's first child, so an act cannot re-parent its strand into an inner container. `Way` and `Proof` were re-composed around it and `Doors` moved to the slot for uniformity. This is what `frames.ts` had claimed since Task 2 while three of five acts did the opposite, with every assertion green — `assertContinuity` compares fractions of each act's own box and cannot see a box. Checked per act, across the composition in `page.test.ts`, and by a probe that moves the slot below the header.
- **One writer per property, and the walk verifies its own scope (R23).** The walk's `onUpdate` now queries `[data-walk-rail]` and checks it found exactly `frame.railSegments.length` paths before writing; it had been writing to all eleven paths in the section, which forced the ribbon's seven undrawn on every tick.
- **Two strands per act where §8 needs a different thread, one displayed (R24).** Acts 0 and 3 render a second `LineStage`: the phone's single fall, and the axis motif's vertical spine below `lg`. §3.1's "one `LineStage` per act" is read as one *visible* stage per act. **Task 12 will meet this** — it mounts a calibrate overlay and reasons about the scrubbed acts.
- **Whether the line is *visually* continuous is still unverified.** The repair makes the boxes right and pins them with checks that fail; no browser was launched and none can be. Everything about how it *looks* — the strand, the axis under its stations, the rail during the walk, the fork on a tablet — is the owner's QA. The single highest-value screenshot remains `rulings.md:215`'s: `preserveAspectRatio` × `pathLength` × `non-scaling-stroke` pinned individually, falsifiable only at a non-1.5:1 aspect ratio.
- **The homepage is enforced by exactly one check, and only when an anchor is wrong (R21).** No test file imports `page.tsx` or `layout.tsx` — so **deleting the `assertContinuity` call** passes the whole gate, and so does **`programmeName: programme.name` → `pillar.name`**, which renders the pillar's name where the programme's belongs: `Station` carries both fields, so the type system accepts it, and the probe measured 334 green with build exit 0. What *is* enforced is the wiring: breaking `doors.enter.x` by 0.01 fails the build with `Strand seam broken at proof.exit → doors.enter`, thrown at module evaluation of `page.tsx:61` — the first time this stage has proved an assertion fires rather than that it exists. Task 12's cross-cutting Step 5 scans the *acts*, not the page — so **the repair closed it**: `src/app/(site)/page.test.ts` renders `Home` under `EnquiryModalProvider`, asserts the five acts in `ACT_ORDER` order, asserts each act's own copy, checks every station against `programmes.ts` (with the two names asserted different, so a swap fails rather than comparing a value to itself), and asserts the strand-box rule for all three `ActSection` acts. Probed: deleting `<Way />` now fails it, naming the act. The two probes that motivated it — the deleted call and the swapped field — are the reason it exists.
...
- **Six stale present-tense citations are corrected, and the inventory that named them was itself wrong.** Task 11's record listed "two comments naming deleted files" — `line/station.ts:32` and `acts/FinalCTA.tsx:25`. The line number was wrong (the comment is at 19–22 and cites `Methodology.tsx`, not `useScrollProgress`), and calling both "historical rather than false" was untrue of that one: it said the old formula "is still live at `Methodology.tsx:29`" and "nothing imports `drawAt` yet", both false the moment the section retired and the walk called it. The repair's Task 8 corrected all six: that one, `station.test.ts`'s copy of it, `frames.test.ts`'s "nothing imports `frames.ts` yet", `anchors.ts`'s "no runtime module calls it", and the three "moved verbatim from" attributions in `Way`, `Proof` and `FinalCTA` — each now says the source retired, and names the **task** that retired it (the plan's step asked for a commit hash; a task reference is what shipped, since the hash is one `git log` away and a task name is stable). **A wrong line number in a finding is as costly as the wrong claim it points at**: the reader who trusts the inventory does not go and look.
- **`src/hooks/useSectionProgress.ts` is still dead code** — no consumer, and none at `HEAD` before Task 11 either. Pre-existing, not orphaned by the retirements, and not on §5's list; it needs an owner's call, not a sweep.
