...
const EcosystemScene = dynamic(() => import("../three/scenes/EcosystemScene"), {
  ssr: false,
});
...
export default function Hero() {
  ...
  const contentOpacity = 1 - Math.min(0.35, progress * 0.1);
  const sceneOpacity = 1 - Math.min(0.25, progress * 1.1);

  const heroDelay = (ms: number) =>
    ({ "--hero-delay": `${ms}ms` }) as CSSProperties;
  ...
  return (
    ...
    >
      ...
      <div
        className="hero-enter-fade absolute inset-0 pointer-events-none"
        style={heroDelay(0)}
      >
      ...
      {/* Mobile atmosphere */}
      <div
        className="hero-enter-fade lg:hidden absolute inset-0 pointer-events-none"
        style={heroDelay(300)}
      >
      ...
      <div className="container-site relative z-10 pt-32 pb-28 md:pt-36">
        ...
        >
          ...
          >
            ...
            <br />
            One learning{" "}
            <span className="text-ec-teal-dark dark:text-ec-teal-light">
              ecosystem
            </span>
            .
          </h1>
          ...
          >
            From language and inclusion to wellbeing, AI literacy, and
            competitive exam preparation — Educraft connects the pieces that
          ...
          <p
            className="hero-enter type-caption text-ec-slate mt-7"
            style={heroDelay(1700)}
          >
            Five verticals · One trust umbrella · Built for schools, families,
            and students
          </p>
        </div>
      </div>
      ...
      <div
        ...
        aria-hidden="true"
      >
        <span className="type-caption uppercase tracking-[0.14em] text-ec-slate">
          Scroll
        </span>
