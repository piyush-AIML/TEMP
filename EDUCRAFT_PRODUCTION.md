...
v2 gives each pillar three measured tiers (`text` / `graphic` / `soft`), verified against **all four light and all three dark canvases**. All text tiers ≥5.00:1 light, ≥6.99:1 dark. Brand chrome splits **teal into text and graphic tiers** (one vivid teal cannot serve both — `#00b3b8` is a correct stroke and a 2.58:1 text), and `excel` (NEET & JEE) moves off brand indigo to **rose `#C2185B`**, resolving the old collision where two of the five "identities" were the brand's own colours. **Corrected during implementation (plan §6.6):** gold is a **fill** family, not a text one — `--ec-gold` is the primary CTA's background, so darkening it for text-AA dropped that pairing from 8.83:1 to 2.64:1; and an earlier draft's `#8E9AE0` dark indigo partner was dropped because nothing painted it while the test asserted its AA.
