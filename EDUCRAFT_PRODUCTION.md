...
- **Code state:** shipped + verified 2026-09-05 — lint ✓ · tsc ✓ · build ✓ (29 routes). **Storage smoke gate PASSED 2026-09-05** against the owner's live bucket `educraftbucket07` (`ap-southeast-2`): presigned PUT 200 → verifyUpload size match → signed GET body match → delete → verify-gone. **Remaining (owner): the browser visual E2E** — professor → course → Materials → File tab → upload a real file → student sees it with a working signed download link (needs a dev-server restart after env changes; Next loads `.env.local` at boot only).
