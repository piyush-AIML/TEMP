...
> **Status:** **Stage 2 — Professor Core shipped 2026-09-05** (details absorbed into §24.8 below). **§24.9 storage blocker RESOLVED at code level 2026-09-05** (provider switched UploadThing → **AWS S3**; lint/tsc/build ✓) — the smoke gate re-run awaits the owner's AWS setup (§24.9.1). **Next: Stage 3 — Meetings & Planner** (unaffected). The project is an authenticated, role-based dashboard module on top of this site — the existing marketing site stays untouched. The kept roadmap plan lives in [`Dashboard-Implementation-Plan.md`](Dashboard-Implementation-Plan.md) — this section is the master-side reference, kept in sync with it (route-group note in §24.4: the file's `(marketing)` label maps to this repo's actual `(site)` group).
...
- **Code state:** shipped + verified 2026-09-05 — lint ✓ · tsc ✓ · build ✓ (29 routes). **Open (owner's AWS side, §24.9.1): create the bucket/IAM/CORS, set the four `S3_*` env vars + `STORAGE_PROVIDER=s3`, then re-run `npm run db:storage-smoke`** — the end-to-end FILE path is not field-verified until that passes.
- **What works already (unchanged from the UploadThing attempt):** the provider-agnostic layer, the direct PUT flow, the generic FileDropzone, and the FILE-row domain/query/UI paths. Env: `S3_REGION`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` (mirrored in `.env.example`); unset/unknown `STORAGE_PROVIDER` still means the honest "uploads disabled" path.

### 24.9.1 AWS setup (owner action — one-time)

1. **Bucket** — create in `S3_REGION` (e.g. `ap-south-1`), name = `S3_BUCKET`. **Block all public access ON** (objects stay private; access is only via presigned URLs).
...
3. **CORS** on the bucket — browsers PUT directly to the presigned URL, so the origin must be allowed (`AllowedHeaders: *` is required so the browser may send the pinned `Content-Type`):
   ...
   [{ "AllowedOrigins": ["http://localhost:3000", "https://temp-tau-opal.vercel.app", "https://educraft.com"],
      ...
      "MaxAgeSeconds": 3000 }]
   ```
4. Set `STORAGE_PROVIDER=s3` in `.env.local`; run `npm run db:storage-smoke`; then do the browser check (professor → course → Materials → File tab → upload → student sees signed download link).
