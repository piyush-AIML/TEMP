...
| **Next project** | Student & Professor **Dashboard** — Stages 0–1 **shipped 2026-09-04**, Stage 2 (Professor Core) **shipped 2026-09-05**; **Stage 3 (Meetings & Planner) is next** (§24) |
...
- **File storage — provider-agnostic by user requirement.** `src/lib/storage/`: `types.ts` (`StorageProvider` interface: `createUpload/verifyUpload/getDownloadUrl/delete` + `StorageNotConfiguredError`), `uploadthing.ts` (UploadThingProvider — THE only `uploadthing/*` import; implements the documented v7 "build your own SDK" server flow: sqids keygen with the published djb2/shuffle recipe, HMAC-SHA256-signed direct ingest URL with `expires`/`x-ut-identifier`/`x-ut-file-name`/`x-ut-file-size`/`x-ut-file-type`/`x-ut-acl=private` — deliberately no `x-ut-slug`, no file route; `verifyUpload` HEADs a short signed URL for real size; downloads via `utapi.generateSignedURL` 24h; deletes via `deleteFiles`), `index.ts` (`getStorage()` factory on `STORAGE_PROVIDER`; unset → `DisabledStorage` with honest UI copy). DB rows store `fileProvider` + opaque `fileKey` + `fileMeta Json {name,size,mime}` — never URLs. Client uploads never touch our server (Vercel's 4.5 MB body cap): `FileDropzone` (generic, provider-agnostic, XHR progress) mints `createUploadToken` (ownership checked BEFORE signing) → PUTs straight to the ingest URL → `completeFileUpload` (verify + persist + notify). S3 migration = future `scripts/storage-migrate.ts` updating provider/key rows only. **Two migrations shipped:** `add_material_file_storage` (StorageProvider enum + 3 Material columns; `fileUrl` documented LINK-only) and `add_user_notification_prefs` (brought forward from Stage 4 — the notification builder reads prefs from day one; `notifPrefs Json?`, absent = all ON; `getCurrentUser` mirror never touches it).
...
- Verified 2026-09-05: lint ✓ · tsc ✓ · build ✓ (13 dynamic dashboard routes + `/api/dashboard/notifications`); DB smoke against live Neon (migration columns, professor counts 5/1/2/3, bogus-course → null, roster incl. prefs shape). **Open gate:** `npm run db:storage-smoke` — implemented and honestly skips until the owner pastes the real `UPLOADTHING_TOKEN` (V7 tab) into `.env.local` and sets `STORAGE_PROVIDER=uploadthing`; full upload→verify→download→delete loop then validates the hand-rolled signing contract.
