...
| **Next project** | Student & Professor **Dashboard** — Stages 0–1 **shipped 2026-09-04**, Stage 2 (Professor Core) **shipped 2026-09-05**, Stage 3 (Meetings & Planner) **shipped 2026-09-05**, Stage 4 (Polish) **shipped 2026-09-05**; **Stage 5 (QA & Deploy) is next** (§24; the course-setup slice decision from the §24 header still stands) |
...
> **Status:** Stages 2–4 shipped 2026-09-05 (§24.8): Professor Core · AWS S3 storage (smoke gate passed; browser E2E remains the owner's check) · invite flow + minimal admin area · Stage 3 Meetings & Planner · **Stage 4 Polish — SHIPPED 2026-09-05** (mobile drawer, student notification-preferences editor, first loading/error/not-found boundaries, access-control audit clean — §24.8). **Open decision (owner): build the course-setup slice — course creation + enrollment UI — before Stage 5, or go straight to Stage 5**; until enrollment UI exists, newly invited users land on an empty dashboard (seed writes the only `CourseProfessors`/`Enrollment` rows today). **Next: Stage 5 — QA & Deploy** (or the course-setup slice, owner's call). The project is an authenticated, role-based dashboard module on top of this site — the existing marketing site stays untouched. The kept roadmap plan lives in [`Dashboard-Implementation-Plan.md`](Dashboard-Implementation-Plan.md) — this section is the master-side reference, kept in sync with it (route-group note in §24.4: the file's `(marketing)` label maps to this repo's actual `(site)` group).
> **Current live deploy (staging/preview domain):** `temp-tau-opal.vercel.app` — running `56327f0` (Stages 3–4) since 2026-09-05, owner-verified working after deploy (an earlier local 404 report was stale-dev-server artifact, not code — §20 operational note). Production domain still unset — see §22 blockers, env var `NEXT_PUBLIC_SITE_URL`.
...
- **Known gap — course assignment has no UI yet (open decision 2026-09-05):** invites create accounts only. Professor↔course is `CourseProfessors` join rows, student↔course is `Enrollment` rows (`@@unique([studentId, courseId])`, status `ACTIVE`) — today **only the seed writes them**. Planned slice (specced §24.6 `courses.ts` — `createCourse`, `enrollStudent`; not started): professor "Enroll student" (email lookup of the adopted User row) on the course Roster tab + admin course creation with professor assignment. Owner decides ordering vs Stage 4 (see §24 header).
